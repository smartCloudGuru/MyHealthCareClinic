...
const SEARCH_DELAY = 400; // Wait 400 ms after user stops typing then, peform search
const ARROW_UP = 38;
const ARROW_DOWN = 40;
const ENTER = 13;

export default class Onl2AutofillInput extends LightningElement {
    // Public properties
    @api label;
    @api required;
    ...
    set selection(initialSelection) {
        ...
        if (this._curSelection) {
            if (!this._curSelection[0]) this._curSelection[0] = {};
            this.value = this._curSelection.length ? this._curSelection[0].id : null;
        }
    }
    ...
    @api set defaultResults(results) {
    ...
    get defaultResults() { return this._defaultSearchResults;}


    @api
    setDefaultResults(results) {
        console.log('def res:', results);
        if (!!results && results.length > 0) {
            this._defaultSearchResults = [...results];
            if (this._searchResults.length === 0) {
                this.setSearchResults(this._defaultSearchResults);
    ...
    handleInput(event) {
        ...
        if (event.target) this.updateSearchTerm(event.target.value);
    }
    ...
    @api validateRequired() {
        let cls = this.template.querySelector('div.slds-form-element').className;

        // added this to add slds-has-error when required
        if (this.required) {
            if (this._curSelection == null || this._curSelection.length === 0) {
                if (cls != null && cls.indexOf('slds-has-error') < 0) {
                    this.template.querySelector('div.slds-form-element').className += ' slds-has-error';
                }
            } else {
                if (cls != null) {
                    this.template.querySelector('div.slds-form-element').className = cls.replace(' slds-has-error', '');
                }
            }
        } else {
            this.template.querySelector('div.slds-form-element').className = cls.replace(' slds-has-error', '');
    ...
    handleClearSelection() {
        ...
        this.setSearchResults(this._defaultSearchResults); //added
        this._hasFocus = false; //added
